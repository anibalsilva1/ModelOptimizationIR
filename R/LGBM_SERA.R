#' A wrapper that calls \code{lightgbm} with weights returned from \code{sigma}.
#'
...
#' @param parameters Additional parameters that are passed to \code{lightgbm}. Check \link[lightgbm]{lightgbm} for more information.
#' @param verbose Prints the error for each iteration. The error printed defaults to the root mean squared error (RMSE).
#' @param ... Additional parameters to be passed into the model. Check \link[lightgbm]{lightgbm} for more information.
#'
...
#' @export
#'
#' @description A wrapper that calls \link[lightgbm]{lightgbm} with weights returned from \code{\link{sigma}}.
#'
#' @details
#'
...
#'
#' @seealso \link[lightgbm]{lightgbm}, \code{\link{sigma}}, \link[IRon]{phi}
#'
#'
#' @examples
#'
...
#'
#' res <- LGBM.sera(formula, train, test)
#' res
#'
...
#' params <- list(min_data = 20, max_depth=5, eta = 10^{-1})
#'
#' res <- LGBM.sera(formula = formula,
#'                  train = train,
#'                  test = test,
#'                  sigma = sigma,
#'                  parameters = params)
...
LGBM.sera <- function(formula,
  ...
  if(is.null(sigma)){
    ...
    phi <- IRon::phi(y = label_train, phi.parms = phi.ctrl)
    sigma <- sigma(phis = phi, steps = steps)
  }
