...
```{r}
y <- train$LNO2
control.points <- matrix(c(1.1, 0, 0, 3.7, 0, 0, 5, 1, 0), byrow = TRUE, ncol=3) # Extreme points above upper fence.
...
```{r}
model_domain_knowledge <- XGBoost.sera(formula = formula, train = train, test = test, sigma = sigma, nrounds = 250, parameters = params)

model_auto <- XGBoost.sera(formula = formula, train = train, test = test, nrounds = 250, parameters = params)

model_unweighted <- wf.XGBoost(formula = formula, train = train, test = test, nrounds = 250, params = params)
...
model_auto <- XGBoost.sera(formula = formula, train = train, test = test, nrounds = 250, parameters = params, I=10)
